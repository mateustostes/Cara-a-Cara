...
function main() {
    ...
    if (nivelDificuldade != null) { // Usuario selecionou o nível de dificuldade na página index.html
        ...
        setInterval(atualizarMsgJogador2, 100);
        //VencedorTempo2();
    }
}

function atualizarMsgJogador2() {
    var indice = parseInt(localStorage.getItem("g_indiceMsgsJogo"));
    document.getElementById("msgJogador2").innerHTML = g_statusJogo[indice].msgJogador2;
}
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
var teste12;
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------
// Selecionar o personagem do jogador 
...
function selecionarPersonagem(vetPosicaoPersonagem) {
    ...
    if (g_personagemJogador.length != 0) {
        if (chute == false) {
            document.getElementById("personagem" + vetPosicaoPersonagem[0] + "" + vetPosicaoPersonagem[1]).src = "cartasJogo/carta0_0.png";
            // O personagem ainda não foi escolhido pelo jogador (primeiro clique dado pelo jogador em qualquer cartinha)
        } else {
            if (localStorage.getItem('Nome jogador 1') == teste12) {
                alert("Acertou!");
        ...
        if (parseInt(localStorage.getItem("g_indiceMsgsJogo")) == 0) {
            alert("Espere o primeiro Jogador escolher a carta!");
        } else {
            var imagem = document.createElement("img");
            ...
            valoresAtributos = Object.values(g_caractPersonagens[g_matrizPersonagens[vetPosicaoPersonagem[0]][vetPosicaoPersonagem[1]] - 1]);
            localStorage.setItem('Nome jogador 2', valoresAtributos[1])
            atributoJogador2 = valoresAtributos[1]
        ...
        for (let coluna = 0; coluna < g_matrizPersonagens[0].length; coluna++) {
            ...
            if (resposta == "sim") {
                if (valoresAtributos[indice] != valorAtributoSelecJogador.value)
                    document.getElementById("personagem" + linha + "" + coluna).style.border = "5px solid rgb(237, 49, 20)";
            } else {
                if (valoresAtributos[indice] == valorAtributoSelecJogador.value)
                    document.getElementById("personagem" + linha + "" + coluna).style.border = "5px solid rgb(237, 49, 20)";
            }
...
var final2 = setInterval(() => {

    if (localStorage.getItem("Tempo") != 0 && localStorage.getItem("Vencedor") == null) {
        document.getElementById('tempoRespostaPainelJogador2').innerHTML = localStorage.getItem("Tempo");

    } else {
        var Vencedor = localStorage.getItem("Vencedor");
        if (Vencedor == 2) {
            document.getElementById('tempoRespostaPainelJogador2').innerHTML = "Venceu!";
            reiniciar2();

        } else if (Vencedor == 1) {
            document.getElementById('tempoRespostaPainelJogador2').innerHTML = "Perdeu!";
            reiniciar2();
        }
...
setInterval(monitorarMsg, 1000);

function monitorarMsg() {
    var msg = localStorage.getItem("msgParaJogador2");
    if (msg != null) {
        document.getElementById("perguntaJogador2").innerHTML = msg;
        document.getElementById("boxPerguntaJogador2").className = "visivel";
    } else {
        document.getElementById("boxPerguntaJogador2").className = "invisivel";
    }
}